...
from .serializer import ServiceSerializer ,ContactUsSerializer ,CareerSerializer , JobApplicationSerializer,VideoSerializer,GallerySerializer
...
class JobApplicationAPIView(APIView):
    def get(self, request):
        applications = JobApplication.objects.all().select_related('career').order_by('-applied_at')
        paginator = PageNumberPagination()
        paginator.page_size =10
        result_page =paginator.paginate_queryset(applications,request)
...
class JobApplicationDetailAPIView(APIView):
    def get(self, request, pk):
        application = get_object_or_404(JobApplication.objects.select_related('career'), pk=pk)
